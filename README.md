...
시장가는 스프레드의 절반을 매번 낸다(BTC 편도 ~0.02%). 지정가(메이커)는 그걸 안 내지만
**체결이 안 될 수 있고**, 실행이 하루 늦으면 −12%p 다([조사](docs/조사-실행타이밍.md)).
그래서 최우선 호가에 지정가를 걸고 `UPBIT_LIMIT_WAIT_SEC`(기본 90초) 기다린 뒤,
미체결분은 **취소하고 시장가로 마무리**한다. 손절·긴급매도는 처음부터 시장가 —
떨어지는 장에서 매도 지정가는 안 잡힌다.

취소 요청과 체결이 경합하면 거래소가 취소를 거부한다. 그때 "0체결"로 믿고 시장가를 또 내면
**두 번 산다.** 취소 결과와 무관하게 재조회한 체결량을 진실로 쓴다 (테스트로 고정).

장부(`reports/fills.csv`)에 `order_type`·`limit_fill_pct` 가 남는다. **모의 모드는 지정가가
100% 잡힌다고 가정**하므로 실제 체결률은 실전에서만 측정된다. `--order-type market` 으로 끌 수 있다.
